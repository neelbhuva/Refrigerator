	...
	for i in range(0,len(zero_indices)):
		if(zero_indices[i] == 0):
			...
			if(i == (len(zero_indices)-1)):
				#print(df["switch"][zero_indices[i]-1])
				df_temp = df["switch"][zero_indices[i]-1] + df["switch"][zero_indices[i]:(len(df["switch"]))]
				df["switch"][zero_indices[i]:(len(df["switch"]))] = df_temp
			else:
				#print(df["switch"][zero_indices[i]-1])
				df_temp = df["switch"][zero_indices[i]-1] + df["switch"][zero_indices[i]:(zero_indices[i+1])]
				#print(df_temp)
				df["switch"][zero_indices[i]:(zero_indices[i+1])] = df_temp
	return df

def doorOpenCount(df,t1,t2):
	...
	#num_of_hours = len(df["switch"])
	#making first energy value 0
	#df = fixDiscontinuity(df)
	#df["switch"] = df["switch"] - df["switch"][0]
	print(df.shape)
	#df['switch'] = str(df['switch'])
	#c1 = df['switch'] == 't'
	print(df["switch"].values.tolist().count(1))
	#print(df["switch"].values.tolist().count(t))
	count = df["switch"].values.tolist().count(1)
	# average = np.average(df["switch"])
	# maximum = np.max(df["switch"])
	# minimum = np.min(df["switch"])
	# var = np.var(df["switch"])
	return [count,num_of_hours]
...
def plot(df,t1,t2,label):
	...
	#print(np.max(x))
	#df["switch"] = df["switch"] - df["switch"][0]
	#print(len(df["switch"]),len(x))
	#df["switch"] = [((9/5)*x+32) for x in df["switch"]]
	plt.hist(df["switch"],label=label)
	plt.ylabel("Frequency of temp")
	plt.xlabel("Inside Temperature in Fahrenheit")
	#plt.axis([x[0], x[len(x)-1], df["switch"][0], df["switch"][len(df["switch"])-1]])

def phase1(df_mario,df_maria,df_odaly):
	#mario
	t1 = dt.datetime.strptime('2017-07-14 14:00:19.445607', '%Y-%m-%d %H:%M:%S.%f')
	t2 = dt.datetime.strptime('2017-07-19 11:00:50.900432', '%Y-%m-%d %H:%M:%S.%f')
	avg_mario = doorOpenCount(df_mario,t1,t2)
	fig1 = plt.figure(1)
	#plot(df_mario,t1,t2,"Without Curtain : Mario")
	fig1.suptitle("Mario")
	#maria
	t1 = dt.datetime.strptime('2017-07-15 04:33:07.203739', '%Y-%m-%d %H:%M:%S.%f')
	t2 = dt.datetime.strptime('2017-07-19 14:00:30.550984', '%Y-%m-%d %H:%M:%S.%f')
	avg_maria = doorOpenCount(df_maria,t1,t2)
	fig2 = plt.figure(2)
	...
	avg_odaly = doorOpenCount(df_odaly,t1,t2)
	fig3 = plt.figure(3)
...
if __name__ == '__main__':
	...
	df_mario = pd.read_csv("mario_switch_table.csv")
	df_maria = pd.read_csv("maria_switch_table.csv")
	df_odaly = pd.read_csv("odaly_switch_table.csv")

	#get required columns
	df_mario = pd.concat([df_mario['datetime'], df_mario['switch']], axis=1)
	df_maria = pd.concat([df_maria['datetime'], df_maria['switch']], axis=1)
	df_odaly = pd.concat([df_odaly['datetime'], df_odaly['switch']], axis=1)
	...
	print("Phase 1 Door open count")
	print("Mario : " + str(a) + " " + "Maria : " + str(b) + "	Odaly : "+ str(c))
	print("Phase 2 Door open count")
