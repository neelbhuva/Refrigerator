	...
	for i in range(0,len(zero_indices)):
		if(zero_indices[i] == 0):
			...
			if(i == (len(zero_indices)-1)):
				#print(df["inside_temp1"][zero_indices[i]-1])
				df_temp = df["inside_temp1"][zero_indices[i]-1] + df["inside_temp1"][zero_indices[i]:(len(df["inside_temp1"]))]
				df["inside_temp1"][zero_indices[i]:(len(df["inside_temp1"]))] = df_temp
			else:
				#print(df["inside_temp1"][zero_indices[i]-1])
				df_temp = df["inside_temp1"][zero_indices[i]-1] + df["inside_temp1"][zero_indices[i]:(zero_indices[i+1])]
				#print(df_temp)
				df["inside_temp1"][zero_indices[i]:(zero_indices[i+1])] = df_temp
	return df
...
def avgMaxMinTemp(df,t1,t2):
	...
	#num_of_hours = len(df["inside_temp1"])
	#making first energy value 0
	#df = fixDiscontinuity(df)
	#df["inside_temp1"] = df["inside_temp1"] - df["inside_temp1"][0]
	print(df.shape)
	average = np.average(df["inside_temp1"])
	maximum = np.max(df["inside_temp1"])
	minimum = np.min(df["inside_temp1"])
	var = np.var(df["inside_temp1"])
...
def plot(df,t1,t2,label):
	...
	#print(np.max(x))
	#df["inside_temp1"] = df["inside_temp1"] - df["inside_temp1"][0]
	#print(len(df["inside_temp1"]),len(x))
	df["inside_temp1"] = [((9/5)*x+32) for x in df["inside_temp1"]]
	plt.hist(df["inside_temp1"],label=label)
	plt.ylabel("Frequency of temp")
	plt.xlabel("Inside Temperature in Fahrenheit")
	#plt.axis([x[0], x[len(x)-1], df["inside_temp1"][0], df["inside_temp1"][len(df["inside_temp1"])-1]])
...
if __name__ == '__main__':
	...
	df_mario = pd.read_csv("mario_inside_temps_table.csv")
	df_maria = pd.read_csv("maria_inside_temps_table.csv")
	df_odaly = pd.read_csv("odaly_inside_temps_table.csv")

	#get required columns
	df_mario = pd.concat([df_mario['datetime'], df_mario['inside_temp1']], axis=1)
	df_maria = pd.concat([df_maria['datetime'], df_maria['inside_temp1']], axis=1)
	df_odaly = pd.concat([df_odaly['datetime'], df_odaly['inside_temp1']], axis=1)
	...
	print("Phase 1 Inside Temp")
	print("Mario : " + str(a) + " " + "Maria : " + str(b) + "	Odaly : "+ str(c))
	print("Phase 2 Inside Temp")
